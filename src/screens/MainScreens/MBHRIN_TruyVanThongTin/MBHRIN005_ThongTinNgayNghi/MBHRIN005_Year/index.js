...
const MBHRIN005_Year = ({ navigation, route }) => {
  ...
  const getData = () => {
    console.log("=== getData called (Year) ===");
    ...
    sysFetch(
      API_URL,
      {
        pro: "STV_HR_SEL_MBI_HRIN005_2_100", // Theo năm
        in_par: in_par,
        out_par: {
          p1_sys: "ngaynghi_year",
        },
      },
      tokenLogin
    )
      .then((rs) => {
        ...
        if (rs === "Token Expired") {
          ...
          setDataNgayNghi(rs.data.ngaynghi_year || []);
        } else {
  ...
  useEffect(() => {
    console.log("=== useEffect triggered (Year) ===");
    console.log("API_URL:", API_URL);
    console.log("tokenLogin:", tokenLogin);
    console.log("userInfo?.empPk:", userInfo?.empPk);
    console.log("selectedMonth:", selectedMonth);
